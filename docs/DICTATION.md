...
浏览器在获得麦克风权限后立即生成 PCM。豆包连接尚未就绪时，最多把 15 秒
音频暂存在当前页面内存中（约 480 KiB），连接成功后按原顺序补发；停止按钮
即使在连接阶段被按下，也会等待连接就绪后提交这段短录音。超时、取消、失败、
离开页面或完成识别都会清空该缓冲，不写入浏览器存储或服务端。
...
WebSocket、点击与长按、连接前音频缓冲、连接中停止、取消、失败保留临时
文字、两分钟停止、TTS 互斥、按钮位置和管理员开关。
